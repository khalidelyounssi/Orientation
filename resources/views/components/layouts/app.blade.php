...
<body class="font-sans antialiased">
    <div class="app-shell lg:flex">
        <aside class="glass-panel border-b border-white/60 px-6 py-6 lg:min-h-screen lg:w-72 lg:border-b-0 lg:border-r">
            ...
            <div class="mt-8 rounded-2xl border border-white/70 bg-white/75 p-4 shadow-sm">
            ...
            <nav class="mt-8 space-y-2">
                ...
                @foreach ($links as $link)
                    ...
                    <a
                        ...
                        class="{{ $active ? 'bg-teal-800 text-white shadow-lg shadow-teal-900/20' : 'text-slate-600 hover:bg-white/80 hover:text-slate-900' }} flex items-center rounded-2xl px-4 py-3 text-sm font-medium transition"
                    >
                        {{ $link['label'] }}
                    </a>
                @endforeach
            ...
            <form method="POST" action="{{ route('logout') }}" class="mt-8">
                @csrf
                <button type="submit" class="w-full rounded-2xl border border-slate-300 bg-white/50 px-4 py-3 text-sm font-medium text-slate-700 transition hover:border-slate-900 hover:text-slate-900">
                    Sign out
                </button>
            </form>
        </aside>
